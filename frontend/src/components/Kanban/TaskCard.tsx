...
interface TaskCardProps {
  ...
  onDeleteTask?: (task: Task) => void; // Add callback for deleting
}

const ItemType = 'TASK';

export function TaskCard({ task, columnId, allTasks = [], onEditTask, onRunTask, onDeleteTask }: TaskCardProps) {
  console.log(`TaskCard ${task.id} - onDeleteTask available:`, !!onDeleteTask);
  ...
  const handleCardClick = (e: React.MouseEvent) => {
    ...
    e.stopPropagation();
    // Don't open edit modal if delete confirmation is showing
  ...
  const handleDeleteClick = (e: React.MouseEvent) => {
    ...
    if (!onDeleteTask) {
      console.log('onDeleteTask function not available');
      return;
    }
  ...
  const handleConfirmDelete = () => {
    console.log('User confirmed deletion, calling onDeleteTask');
    if (onDeleteTask) {
      onDeleteTask(task);
    }
    // Close the dialog after a small delay to prevent event conflicts
    setTimeout(() => {
  ...
  return (
    ...
    >
      ...
      <ConfirmDialog
        ...
        title="Delete Task"
        message={`Are you sure you want to delete task #${task.id}: "${task.title}"? This action cannot be undone and will also clean up any associated git worktree.`}
        confirmText="Delete"
        cancelText="Cancel"
